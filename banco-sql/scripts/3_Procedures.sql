...
END
GO
CREATE PROCEDURE RETORNA_DADOS_HOME 
AS BEGIN
   SELECT 
      VEICULOS_OPERACAO =  ( SELECT COUNT(*) FROM CVEICULOS ),
	  VEICULOS_MANUTENCAO_MES =  ( SELECT COUNT(DISTINCT CMPLACAVEICULO) FROM CMANUTENCAO ),
	  VALOR_MULTA_MES =  ( SELECT SUM(CMVALOR) FROM CMULTAS ),
	  VALOR_MANUTENCAO = ( SELECT COUNT(CMPLACAVEICULO) FROM CMANUTENCAO ) * 142.45,
	  VALOR_PECAS = ( SELECT COUNT(*) FROM TPECAS ) * 342.45,
	  VALOR_ESTACIONAMENTO = ( SELECT SUM(CRVALOR) FROM CREEMBOLSOS WHERE CRTIPO = 1),
